...
export const differentialNodes: Record<string, StoryNode> = {
  ...
  DX_PSYCH_REVIEW: { id: "DX_PSYCH_REVIEW", blocks: [p("가설은 기록된다. 환자의 변화는 계속 재평가해야 한다.")], choices: go("CASE_003", "새 정보와 함께 다시 검토한다") },
  ...
  WAIT_001: { id: "WAIT_001", title: "PBG / ALA pending", blocks: [p("하린의 근력저하는 진행 중이다."), thought("decision", "검사 결과가\n치료 시점을 정해주는 건 아니다."), thought("reasoning", "확신과 성급함은 다르다."), thought("empathy", "틀릴 경우 위험을 감당하는 건 환자다.")], choices: [{ id: "wait", label: "환자의 상태를 지켜본다", terminal: true }] },
};
