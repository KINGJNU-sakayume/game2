import type { Effect, StoryNode } from "@/game/types";
import { d, flag, go, p, s, setFlag, thought } from "./helpers";

const primary = (value: string): Effect[] => [
  { type: "value", key: "primary_diagnosis", value },
  { type: "primaryDiagnosis", diagnosisId: value },
];
export const differentialNodes: Record<string, StoryNode> = {
  ...
  TEST_PBG_01: { id: "TEST_PBG_01", blocks: [s("Spot urine.\n급성 발작 중 생화학적 확인을 위한 검사."), d("강수진", "“차광해서 바로 보낼게요.”"), thought("observation", "색으로 진단하는 병은 아니다."), thought("reasoning", "숫자로 확인한다.")], onEnter: [setFlag("pbg_ordered"), { type: "test", patientId: "harin", testId: "urine_pbg_ala", status: "pending" }, { type: "timeline", entry: { id: "pbg-ordered", kind: "test", text: "PBG / ALA ordered" } }], choices: go("WAIT_001") },
